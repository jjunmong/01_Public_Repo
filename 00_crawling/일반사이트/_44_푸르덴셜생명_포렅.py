...
def main():
    ...
    outfile = codecs.open('44_푸르덴셜생명.txt', 'w', 'utf-8')
    outfile.write("INPUT|ID|BRANCH|OLDADDR|NEWADDR|TELL|CAT|XCORD|YCORD|URL|CAR_ENT|WALK_ENT|MENU|TIME\n")

    inputNames = '푸르덴셜생명'
    # sidoList = ['서울', '경기', '강원', '충북', '충남', '경북', '경남', '전북', '전남', '인천', '대전', '울산', '광주', '대구', '부산', '세종', '제주']

    result = getStoreInfo(inputNames)
...
def getStoreInfo(inputName):
    url ="https://map.naver.com/v5/api/search?caller=&query={}&type=all&searchCoord=127.10515475393677;37.50626&page=1&displayCount=300&isPlaceRecommendationReplace=true&lang=ko".format(inputName)
    headers = {
    ...
    for ss in data_all:
        try:
            id = ss['id']
            branch = ss['name']
            branch = str(branch).replace('푸르덴셜 ','푸르덴셜생명').replace('푸르덴셜생명보험','푸르덴셜생명')
            oldaddr = ss['address']
            ...
            if branch.startswith('푸르덴셜') == False : pass
            else:
