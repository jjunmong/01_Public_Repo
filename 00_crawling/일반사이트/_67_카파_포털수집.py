...
def main():
    ...
    outfile = codecs.open('67_카파.txt', 'w', 'utf-8')
    ...
    for sido in sidoList:
        result = getStoreInfo(sido,inputNames)
        print(sido, inputNames)
    ...
    outfile.close()
def getStoreInfo(sido,inputName):
    url ="https://map.naver.com/v5/api/search?caller=&query={} {}&type=all&searchCoord=127.10515475393677;37.50626&page=1&displayCount=300&isPlaceRecommendationReplace=true&lang=ko".format(sido,inputName)
    headers = {
    ...
    for ss in data_all:
        try:
            ...
            if branch.startswith('카파') == False : pass
            else:
