...
class Ui_Collect_Manager(object):
    ...
    def cliclkReserveStart(self):
        schedule.every().day.at('09:02').do(self.getRepeatList)
    ...
    def getRepeatList(self):#예약 실행 버튼 클릭 시 crawl_schedule_config.txt 에서 값을 읽어와서 예약 실행 수행.
        try:
            ...
            def exportRunList():#exe 리스트에 맞는 config값을 더해서 리스트로 반환.
                ...
                with open('crawl_schedule_config_test.txt', 'r', encoding='utf-8') as config_data:
            ...
            def reserveCrawl(month, week, dayOfweek, day_info, time, exe):  # param 값을 받아서 예약 실행.
                ...
                try:
                    ...
                    # sched.start()
                    if month == '매월' and int(day_info) == int(today_day): schedule.every().day.at(time_info).do(run_crawl,exe),self.textBrowser.append(exe + ' | '+str(date.today())+' | '+'매월/'+ today_date +'/수집 예약 완료')
                    elif week == '매주' and dayOfweek == today_date:
                        schedule.every().day.at(time_info).do(run_crawl,exe), self.textBrowser.append(exe + ' | ' + str(date.today()) + ' | ' + '매주/' + today_date + '/수집 예약 완료')
                    elif week == '1/3주' and weekNum in (1, 3) and dayOfweek == today_date:
                        schedule.every().day.at(time_info).do(run_crawl,exe), self.textBrowser.append(exe + ' | ' + str(date.today()) + ' | ' + '1/3주 수집 예약 완료')
                    elif week == '2/4주' and weekNum in (2, 4) and dayOfweek == today_date:
                        schedule.every().day.at(time_info).do(run_crawl,exe), self.textBrowser.append(exe + ' | ' + str(date.today()) + ' | ' + '2/4주 수집 예약 완료')
                    elif week == '1주' and weekNum == 1 and dayOfweek == today_date:
                        schedule.every().day.at(time_info).do(run_crawl,exe), self.textBrowser.append(exe + ' | ' + str(date.today()) + ' | ' + '1주차 수집 예약 완료')
                    elif week == '2주' and weekNum == 2 and dayOfweek == today_date:
                        schedule.every().day.at(time_info).do(run_crawl,exe), self.textBrowser.append(exe + ' | ' + str(date.today()) + ' | ' + '2주차 수집 예약 완료')
                    elif week == '3주' and weekNum == 3 and dayOfweek == today_date:
                        schedule.every().day.at(time_info).do(run_crawl,exe), self.textBrowser.append(exe + ' | ' + str(date.today()) + ' | ' + '3주차 수집 예약 완료')
                    elif week == '4주' and weekNum == 4 and dayOfweek == today_date:
                        schedule.every().day.at(time_info).do(run_crawl,exe), self.textBrowser.append(exe + ' | ' + str(date.today()) + ' | ' + '4주차 수집 예약 완료')
                    # elif week == '매주' and dayOfweek == today_date: sched.add_job(run_crawl, "date", run_date=run_time,args=(exe,)),self.textBrowser.append(exe + ' | '+str(date.today())+' | '+'매주/'+ today_date +'/수집 예약 완료')
                    # elif week == '1/3주' and weekNum in (1,3) and dayOfweek == today_date: sched.add_job(run_crawl, "date", run_date=run_time,args=(exe,)),self.textBrowser.append(exe + ' | '+str(date.today())+' | '+'1/3주 수집 예약 완료')
                    # elif week == '2/4주' and weekNum in (2,4) and dayOfweek == today_date: sched.add_job(run_crawl, "date", run_date=run_time,args=(exe,)),self.textBrowser.append(exe + ' | '+str(date.today())+' | '+'2/4주 수집 예약 완료')
                    # elif week == '1주' and weekNum == 1 and dayOfweek == today_date: sched.add_job(run_crawl, "date", run_date=run_time,args=(exe,)),self.textBrowser.append(exe + ' | '+str(date.today())+' | '+'1주차 수집 예약 완료')
                    # elif week == '2주' and weekNum == 2 and dayOfweek == today_date: sched.add_job(run_crawl, "date", run_date=run_time,args=(exe,)),self.textBrowser.append(exe + ' | '+str(date.today())+' | '+'2주차 수집 예약 완료')
                    # elif week == '3주' and weekNum == 3 and dayOfweek == today_date: sched.add_job(run_crawl, "date", run_date=run_time,args=(exe,)),self.textBrowser.append(exe + ' | '+str(date.today())+' | '+'3주차 수집 예약 완료')
                    # elif week == '4주' and weekNum == 4 and dayOfweek == today_date: sched.add_job(run_crawl, "date", run_date=run_time,args=(exe,)),self.textBrowser.append(exe + ' | '+str(date.today())+' | '+'4주차 수집 예약 완료')
                    else: pass
                except:
                    err = traceback.format_exc()
                    QMessageBox.warning(Collect_Manager, 'Error', self.errorLog(str(err)))
