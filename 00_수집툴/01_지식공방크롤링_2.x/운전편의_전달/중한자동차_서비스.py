...
def main():
    ...
    outfile = codecs.open('zhmotors_svc_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|SUBNAME|TELNUM|NEWADDR|X|Y@@중한자동차서비스\n")

    # 동풍소콘 정비
    page = 1
    while True:
        store_list = getStores2('302', page)
        if store_list == None: break;

        for store in store_list:
            #outfile.write(u'중한자동차서비스|')
    ...
    page = 1
    while True:
        store_list = getStores2('303', page)
        if store_list == None: break;
...
# v2.0
def getStores2(menukey, intPageNo):
    # 'http://www.zhmotors.com/home/sub.php?menukey=302&page=2&scode=00000000'
    ...
    for i in range(len(entity_list)):
        ...
        store_info['name'] = '신원CK모터스'
        if menukey == '302': store_info['name'] += '/동풍소콘정비'
        elif menukey == '303': store_info['name'] += '/북기은상정비'

        store_info['subname'] = ''
        strtemp = "".join(name_list[0].itertext())
        if strtemp != None:
            strtemp = strtemp.replace('\r', '').replace('\t', '').replace('\n', '').replace('㈜', '').replace(' & ', '&').rstrip().lstrip()
            if strtemp.startswith('('):     # '(주)', '(유)' 같은 것들 앞에서 떼어내기
                idx = strtemp.find(')')
                if idx != -1:
