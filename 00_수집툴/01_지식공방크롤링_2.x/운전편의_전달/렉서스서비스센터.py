...
def main():
    ...
    outfile = codecs.open('lexus_svc_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|SUBNAME|TELNUM|NEWADDR|FEAT|WEBSITE|XCOORD|YCOORD@@렉서스서비스센터\n")
...
def getStores(intPageNo, postfix):
    ...
    api = '/data/json/getData_service.php?dealer=official&page_id=svc_center'
    try:
        urls = url + api
        print(urls)
    ...
    # print(response)
    location_list = json.loads(response)['service']['service_center']['locations']
    ...
    for i in range(len(location_list)):
        ...
        for j in range(len(entity_list)):
            ...
            store_info['feat'] = entity_list[j]['service']

            strtemp = entity_list[j]['link_dealer'].lstrip().rstrip()
            if not strtemp.startswith('http'): strtemp = 'http://lexus.co.kr' + strtemp
