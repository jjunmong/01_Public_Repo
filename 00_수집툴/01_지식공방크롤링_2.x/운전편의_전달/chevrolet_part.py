...
def main():
    ...
    outfile = codecs.open('chevrolet_part_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|SUBNAME|TELNUM|NEWADDR|TELNUM2|FEAT|XCOORD|YCOORD@@쉐보레부품대리점\n")
    ...
    while True:
        ...
        if page == 99: break        # 2019년1월 47page까지 있음
...
def getStores(intPageNo):
    # 'https://www.chevrolet.co.kr/chevy/part-store.gm?
    url = 'https://www.chevrolet.co.kr'
    api = '/chevy/part-store.gm?'
    data = {
        'accessType': 'search',
        'addrSi': '',
        'addrGu': '',
    ...
    for i in range(len(entityList)):
        ...
        strtemp = strtemp.replace('(주)', '').replace('(유)', '').replace('주식회사', '').replace('유한회사', '').lstrip().rstrip()
        ...
        if len(subinfo_list) > 0:
            ...
            if len(feat_list) >= 7:
                # index 0 에 id로 추정되는 값 있음 (필요할 때 추출할 것)
