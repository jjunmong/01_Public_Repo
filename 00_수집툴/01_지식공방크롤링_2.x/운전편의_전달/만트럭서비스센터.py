...
def main():
    ...
    outfile = codecs.open('mantruck_svc_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|SUBNAME|TELNUM|NEWADDR|OT@@만트럭서비스센터\n")
...
def getStores(intPageNo):
    ...
    for i in range(len(name_list)):
        ...
        if strtemp != None:
            strtemp = strtemp.replace('\r', '').replace('\t', '').replace('\n', '').replace('전화|팩스:', '').rstrip().lstrip()
            store_info['pn'] = strtemp.replace('.', '-').replace('|', ';').replace(' ', '')

        store_info['ot'] = ''
        strtemp = "".join(info_list[3].itertext())
        if strtemp != None:
            strtemp = strtemp.replace('\r', '').replace('\t', '').replace('\n', '').replace('영업시간:', '').rstrip().lstrip()
            store_info['ot'] = strtemp.replace('.', '-').replace('|', ';')
