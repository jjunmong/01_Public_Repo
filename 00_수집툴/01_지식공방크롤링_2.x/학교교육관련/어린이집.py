...
def main():
    ...
    outfile = codecs.open('kindergarten3_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|SUBNAME|TYPE|ID|TELNUM|NEWADDR|OT|XCOORD|YCOORD@@어린이집\n")
...
def getStores(intPageNo):
    ...
    for i in range(len(entity_list)):
        ...
        if strtemp != None:
            ...
            strtemp = strtemp.replace('(복)', '').replace('(신)', '').replace('(재)', '').replace('(주)', '').rstrip().lstrip()
        ...
        # 상세정보 페이지에서 전화번호 정보 얻을 수 있음 (시간 많이 걸려서 일단 호출하지 않음, 4만여개 있음 ㅠㅠ)
        if store_info['id'] != '':
