...
def main():
    ...
    outfile = codecs.open('br31_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|SUBNAME|TELNUM|NEWADDR|OT@@배스킨라빈스\n")
    ...
    for sidoname in sorted(sidolist):
        ...
        for gugunname in sorted(gugunlist):
            ...
            for store in storeList:
                outfile.write(u'배스킨라빈스|')
                subname = store['name']
                if not subname.endswith('점'): subname += '점'
                outfile.write(u'%s|' % subname)
                outfile.write(u'%s|' % store['tel'])

                store_addr = store['address'].replace('&#40;', '(').replace('&#41;', ')').replace('.1층', ' 1층').replace('.2층', ' 2층')
                outfile.write(u'%s|' % store_addr)

                store_ot = ''
                if store.get('operationtime'): store_ot = store['operationtime'].replace('&#58;', ':').replace(' ', '')
                outfile.write(u'%s\n' % store_ot)
