...
def main():
    ...
    outfile = codecs.open('funeral_ehaneul_all_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|SUBNAME|ID|TELNUM|NEWADDR|TYPE|ETC|FEAT|XCOORD|YCOORD|SOURCE2@@장례식장\n")

    outfile2 = codecs.open('funeral_hospital_utf8.txt', 'w', 'utf-8')
    outfile2.write("##NAME|SUBNAME|ID|TELNUM|NEWADDR|TYPE|ETC|FEAT|XCOORD|YCOORD|SOURCE2@@병원장례식장\n")

    outfile3 = codecs.open('funeral_other_utf8.txt', 'w', 'utf-8')
    outfile3.write("##NAME|SUBNAME|ID|TELNUM|NEWADDR|TYPE|ETC|FEAT|XCOORD|YCOORD|SOURCE2@@일반장례식장\n")
    ...
    while True:
        ...
        for store in store_list:
            ...
            if store['type'] == '병원장례식장':
                ...
                outfile2.write(u'%s|' % store['etcinfo'])
                outfile2.write(u'%s|' % store['feat'])
                outfile2.write(u'%s|' % store['xcoord'])
                outfile2.write(u'%s|' % store['ycoord'])
                outfile2.write(u'%s\n' % u'보건복지부장사정보시스템')
            else:
                ...
                outfile3.write(u'%s|' % store['etcinfo'])
                outfile3.write(u'%s|' % store['feat'])
                outfile3.write(u'%s|' % store['xcoord'])
                outfile3.write(u'%s|' % store['ycoord'])
                outfile3.write(u'%s\n' % u'보건복지부장사정보시스템')

        page += 1

        if page == 199: break       # 2018년5월 112까지 있음
...
def getStores(intPageNo):
    ...
    api = '/portal/fnlfac/facList.ajax'
    data = {
        'facilityDivCd': '1',   # 1=장례식장
        'pageInqCnt': '',
        'sidoCd': '',
        'gunguCd': '',
        'publicCode': '',
    ...
    params = 'facilityDivCd=1&curPageNo=' + str(intPageNo) + '&sidoCd=&gunguCd=&publicCode=&companyName=&latitude=37.5723772025&longitude=127.0142857103'
    print(params)
    ...
    for i in range(len(entity_list)):
        ...
        if strtemp != None:
            ...
            if strtemp.find('병원') != -1:
                store_info['type'] = '병원장례식장'
            elif strtemp.find('의료원') != -1:
                store_info['type'] = '병원장례식장'
        ...
        strtemp = entity_list[i]['funeraltypecd']
        if strtemp == '병원': store_info['type'] = '병원장례식장'
        else: store_info['type'] = ''
