...
def main():
    ...
    while True:
        ...
        if ot_idx != -1: store_ot = word_list[ot_idx].replace('\r', '').replace('\t', '').replace('\n', '').rstrip().lstrip()
        ...
        store_ot = store_ot.replace(u'\u0101', 'a').replace(u'\u9ba8', '').replace(u'\xe9', 'e').replace(u'\xe0', 'a').replace(u'\xa0', '')\
            .replace(u'\xc9', 'e').replace(u'\xea', 'e').replace(u'\xf4', 'o').replace(u'\xe7', 'c').replace(u'\xe8', 'e')\
            .replace(u'\u2013', ' ').replace(u'\u22c5', ' ').replace(u'\u2027', ' ').replace(u'\u30fb', ' ').replace(u'\uff65', ' ')\
        ...
        store_pn = store_pn.replace('', '').replace(u'\xa0', ' ').replace( u'\u2013', '').replace(u'\u200b', '') \
            .replace(u'\ufffd', '').replace(u'\u0223', '').replace(u'\u202c', '').replace(u'\u2010', '-')
        ...
        # temporary (2019/1/21) <= 맵퍼스에서 ot 정보 요청
        store_feat2 = store_ot
