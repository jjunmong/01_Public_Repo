...
def main():
    ...
    outfile = codecs.open('prayer_center_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|FEAT|SUBNAME|TELNUM|NEWADDR|FATHER|SOURCE2@@기도원\n")
    ...
    while True:
        ...
        if page == 199: break      # 2018년9월 기준 59까지 있음
...
def getStores(intPageNo):
    ...
    for i in range(len(entity_list)):
        ...
        if strtemp != None:
            ...
            if strtemp.endswith('수양관'): pass
            elif strtemp.endswith('수양원'): pass
            elif strtemp.endswith('수련관'): pass
            elif strtemp.endswith('기도원'): pass
            elif strtemp.endswith('평화원'): pass
            elif strtemp.endswith('훈련원'): pass
            elif strtemp.endswith('수련'): pass
            elif strtemp.endswith('훈련'): pass
            elif strtemp.endswith('센터'): pass
            elif not strtemp.endswith('기도원'): strtemp += '기도원'
        ...
        if strtemp != None:
            ...
            if idx != -1:
                ...
                store_info['pn'] = strtail.replace('.', '-').replace('|', '-').replace(')', '-').replace(' ', '')
            ...
            if idx != -1:
                strtail = strtemp[idx+2:].lstrip()
                if strtail.startswith(':'): strtail = strtail[1:].lstrip()
                strtail = strtail.replace('|', '-').lstrip().rstrip()
                if strtail.endswith('-'): strtail = strtail[:-1].rstrip()
                store_info['newaddr'] = strtail
                strtemp = strtemp[:idx].rstrip()

            idx = strtemp.rfind('원장')
            if idx != -1:
                strtail = strtemp[idx+2:].lstrip()
                if strtail.startswith(':'): strtail = strtail[1:].lstrip()
