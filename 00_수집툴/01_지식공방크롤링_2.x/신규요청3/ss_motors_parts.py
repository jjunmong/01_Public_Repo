...
def main():
    ...
    outfile = codecs.open('ss_motors_parts_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|SUBNAME|TELNUM|ADDR|NEWADDR@@르노삼성자동차부품\n")
...
def getStores(strSidoName):
    ...
    api = '/2016/customer/search/getOfficeList.jsp'
    data = {
        'search_type': 'addr'
    }
    ...
    for i in range(len(entityList)):
        ...
        storeInfo = {}
        storeInfo['name'] = '르노삼성부품'      # '르노삼성부품'을 속성으로 하고, subname을 name으로 처리할까???
        ...
        if subname.startswith('르노삼성부품'): subname = subname[6:].lstrip()

        if subname.startswith('르노삼성자동차'): subname = subname[-7:].lstrip()
        elif subname.startswith('르노삼성센터'): pass
        elif subname.startswith('르노삼성'): subname = subname[-4:].lstrip()

        if subname.startswith('(') and subname.endswith(')'): subname = subname[1:-1].lstrip().rstrip()
