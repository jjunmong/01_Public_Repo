...
def main():
    ...
    outfile = codecs.open('stock_hanyang_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|SUBNAME|TELNUM|NEWADDR@@한양증권\n")
...
def getStores(intPageNo):
    url = 'http://www.hygood.co.kr'
    api = '/contents/06/0607'
    data = {}
    ...
    entity_list = tree.xpath('//div[@class="agent_list fr"]//tbody//tr')

    store_list = []
    for i in range(len(entity_list)):
        name_list = entity_list[i].xpath('.//th')
        info_list = entity_list[i].xpath('.//td')
        if len(name_list) < 1 or len(info_list) < 1: continue  # 최소 필드 수 체크

        store_info = {}
        store_info['name'] = '한양증권'
        ...
        # 상세정보 페이지에 x,y 좌표 있음 (필요할 때 추출할 것!)
