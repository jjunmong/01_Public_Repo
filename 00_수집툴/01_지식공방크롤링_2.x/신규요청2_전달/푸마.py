...
def main():
    ...
    outfile = codecs.open('puma_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|SUBNAME|TYPE|TELNUM|NEWADDR|FEAT|XCOORD|YCOORD@@푸마\n")
...
def getStores(intPageNo):
    ...
    api = '/customer/account/findstore_list'
    #data = {}
    ...
    entity_list = tree.xpath('//div[@class="puma_str_name"]')
    ...
    for i in range(len(entity_list)):
        ...
        if strtemp != None:
            strtemp = strtemp.replace('\r', '').replace('\t', '').replace('\n', '').replace('_', ' ').rstrip().lstrip()
            if strtemp.startswith('푸마'): strtemp = strtemp[2:].lstrip()
        ...
        if strtemp != None:
            # '[백화점] 매장 블로그'와 같은 형식으로 유형 정보 입력되어 있음 <= 여기서 [] 안에 있는 '백화점' 추출
            strtemp = strtemp.replace('\r', '').replace('\t', '').replace('\n', '').rstrip().lstrip()
            idx = strtemp.find(']')
        ...
        temp_list = info_list[0].xpath('.//span[@class="puma_str_title"]/@lat')
        if len(temp_list) > 0:
            strtemp = temp_list[0]
            strtemp = strtemp.replace('\r', '').replace('\t', '').replace('\n', '').rstrip().lstrip()
            store_info['ycoord'] = strtemp
        temp_list = info_list[0].xpath('.//span[@class="puma_str_title"]/@lng')
        if len(temp_list) > 0:
            strtemp = temp_list[0]
            strtemp = strtemp.replace('\r', '').replace('\t', '').replace('\n', '').rstrip().lstrip()
