...
def main():
    ...
    outfile = codecs.open('kappa_utf8.txt', 'w', 'utf-8')
    outfile.write("##NAME|SUBNAME|TELNUM|NEWADDR|XCOORD|YCOORD@@카파\n")
...
def getStores(intPageNo):
    url = 'http://www.kappakorea.net'
    api = '/company/store.asp'
    data = {
        'cate': '전체보기',
        #'cate': '',
        'searchKeyword': '',
    ...
    entity_list = tree.xpath('//ul[@class="store_list"]//li//dt')
    ...
    for i in range(len(entity_list)):
        ...
        if len(temp_list) > 0:
            strtemp = temp_list[0]
            idx = strtemp.find('store_view(')
            if idx != -1:
                strtemp = strtemp[idx+11:]
                idx = strtemp.rfind(');')
                subinfo_list = strtemp[:idx].split(',')
                if len(subinfo_list) >= 2:
                    store_info['xcoord'] = subinfo_list[0].replace('\'', '').lstrip().rstrip()
                    store_info['ycoord'] = subinfo_list[1].replace('\'', '').lstrip().rstrip()
