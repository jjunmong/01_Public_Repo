...
def main():
    ...
    outfile = codecs.open('24_chevrolet_dealer.txt', 'w', 'utf-8')
    outfile.write("##NAME|BRANCH|XCOORD|YCOORD|ADDR|ADDR_INFO|TELL|ID\n")
    ...
    for store in new_results:
        ...
        outfile.write(u'%s|' % store['addr_info'])
...
def getStore_num():
    url = 'https://www.chevrolet.co.kr/purchase/dealer-location.gm?_csrf=75813b3e-4afd-47e6-9b2e-e73031be935d&accessType=search&addrCi=&addrGu=&searchType=1&searchCity=0&searchGu=&mgmtDeptName=%C0%FC%BD%C3%C0%E5%B8%ED%C0%BB+%C0%D4%B7%C2%C7%CF%BC%BC%BF%E4+%BF%B9%29%B5%BF%BC%AD%BF%EF&pgIdx=1'
    try:
...
def getStore_info(intPageNum):
    url = 'https://www.chevrolet.co.kr/purchase/dealer-location.gm?_csrf=75813b3e-4afd-47e6-9b2e-e73031be935d&accessType=search&addrCi=&addrGu=&searchType=1&searchCity=0&searchGu=&mgmtDeptName=%C0%FC%BD%C3%C0%E5%B8%ED%C0%BB+%C0%D4%B7%C2%C7%CF%BC%BC%BF%E4+%BF%B9%29%B5%BF%BC%AD%BF%EF&pgIdx={}'.format(intPageNum)
    try:
        urlOpen = requests.get(url)
        print(urlOpen, url)
    except:
    ...
    for i in range(len(entityList)):
        ...
        storeInfo['addr_info'] = info_list_split[5].lstrip().rstrip().upper()
        storeInfo['tell'] = info_list_split[6].lstrip().rstrip().upper()
