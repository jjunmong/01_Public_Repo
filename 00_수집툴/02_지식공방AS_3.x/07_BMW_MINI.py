...
outfile = codecs.open('07_BMW_MINI.txt', 'w', 'utf-8')
outfile.write("NAME|BRANCH|ADDR|TELL|OT|BRAND\n")
...
def parse(pageString):
    ...
    for list in tr:
        try:
            ...
            data.append({"name":name,"branch":branch,"addr":addr,"tell":tell,"time":time,"brand":brand})
    return data

def getInfo():
    url = "https://kr.mini.co.kr/map/showRoom"
...
for result_list in new_results:
    ...
    outfile.write(u'%s|' % result_list['time'])
    outfile.write(u'%s|\n' % result_list['brand'])
outfile.close()