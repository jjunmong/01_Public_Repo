...
today_fix = str(datetime.today()).split(' ')[0].replace('-', '')
if os.path.exists('수집결과\\08_1_도로명주소\\') == False : os.makedirs('수집결과\\08_1_도로명주소\\')
outfilename_true = '수집결과\\08_1_도로명주소\\도로명주소_{}.log_성공.txt'.format(today_fix)
outfilename_false = '수집결과\\08_1_도로명주소\\도로명주소_{}.log_실패.txt'.format(today_fix)
...
def download_addrDB():
    ...
    if '파일을 찾을 수 없습니다' in response_text :
        ...
        month_origin = today_info.month - 1
        year = str(today_info.year)
        if today_info.month == 1 :
            month_origin = 12
            year = str(today_info.year-1)
        if len(str(month_origin)) < 2: month = '0' + str(month_origin)
        else : month = str(month_origin)
        fulldate = year + month
        print(fulldate)
...
def download_PObox():
    ...
    file_name = ''
    for s in file_list:
        if s.endswith('_사서함주소DB_전체분.zip') == True:
            file_name = s

    origin_file = origin_dir + '\\' + file_name
    copy_file = current_dir + '\\수집결과\\'+'08_1_도로명주소'+'\\'+ file_name
    shutil.copy(origin_file, copy_file)
    outfilename = copy_file.replace('.zip','')+'_'+(today_fix)+'.zip'
